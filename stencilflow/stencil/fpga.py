...
class ExpandStencilXilinx(dace.library.ExpandTransformation):
    ...
    def expansion(node, parent_state, parent_sdfg):
        ...
        for field_name, (dim_mask, relative) in node.accesses.items():
            ...
            # between this index and the next. Leave out the last vector length
            # accesses, as they will be set directly from the wavefront
            buffer_sizes[field_name] = [
                buffer_accesses[field_name][1][i + 1] -
                buffer_accesses[field_name][1][i] for i in range(
                    len(buffer_accesses[field_name][1]) - vector_length)
            ]
        ...
        for name, (accesses, _, _) in buffer_accesses.items():
            connectors = [
                code_memlet_names[name][a] for a in accesses
            ]
