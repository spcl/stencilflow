...
class ExpandStencilXilinx(dace.library.ExpandTransformation):
    ...
    def expansion(node, parent_state, parent_sdfg):
        ...
        # to the center
        init_sizes = [sum(buffer_sizes[key], 0) - val["center"] for key, val in buffer_accesses.items()]
        ...
        for name, accesses in buffer_accesses.items():
            ...
            out_connectors.extend(f"{c}_out" for c in connectors[:-vector_length])

        compute_tasklet = state.add_tasklet(node.label,
                                            in_connectors,
                                            out_connectors,
                                            code,
        ...
        for (field_name, sizes), init_size in zip(buffer_sizes.items(), init_sizes):
            ...
            _, buffer_desc = sdfg.add_array(buffer_name_vector, (1, ),
            ...
            _, buffer_desc = sdfg.add_array(buffer_name_scalar, (vector_length, ),
...
if {it} >= {begin} and {it} < {end}:
    ...
    {conn_out} = {junk}""".format(it=pipeline.iterator_str(),
            ...
            for i, (size, acc) in enumerate(zip(sizes, buffer_accesses[field_name]["major_unique"][:-1])):
                ...
                output_cast_name = f"{field_name}_output_cast_{i}"

                field_dtype = parent_sdfg.data(data_name).dtype
                _, desc_vector = sdfg.add_array(buffer_name, (size, ),
                                                dace.vector(field_dtype.base_type, vector_length),
                                                storage=dace.dtypes.StorageType.FPGA_Registers,
                                                transient=True)
                _, desc_input_cast = sdfg.add_array(input_cast_name, (vector_length, ),
                                                    field_dtype.base_type,
                                                    storage=dace.dtypes.StorageType.FPGA_Registers,
                                                    transient=True)
                _, desc_output_cast = sdfg.add_array(output_cast_name, (vector_length, ),
                                                     field_dtype.base_type,
                                                     storage=dace.dtypes.StorageType.FPGA_Registers,
                                                     transient=True)
                ...
                input_cast = state.add_access(input_cast_name)
                output_cast = state.add_access(output_cast_name)

                buffer_subset = f"{buffer_name}[{pipeline_iterator}%{size}]"
                ...
                for r, m in zip(relative, minor):
                    ...
                    state.add_memlet_path(input_cast,
                                          compute_tasklet,
                                          dst_conn=f"{conn}_in",
                                          memlet=dace.Memlet(f"{input_cast_name}[{m}]"))
                    state.add_memlet_path(compute_tasklet,
                                          output_cast,
                                          src_conn=f"{conn}_out",
                                          memlet=dace.Memlet(f"{output_cast_name}[{m}]"))
                ...
                # Cast to buffer
                state.add_memlet_path(output_cast,
                                      exit,
                                      buffer_write,
                                      memlet=dace.Memlet(buffer_subset, other_subset=cast_subset))
        ...
        for field_name, offset in node.output_fields.items():
            ...
            _, output_buffer_scalar_desc = sdfg.add_array(output_buffer_scalar_name, (vector_length, ),
                                                          stream.dtype.base_type,
                                                          storage=dace.StorageType.FPGA_Registers,
                                                          transient=True)
            output_buffer_scalar_access = state.add_access(output_buffer_scalar_name)

            # Add intermediate buffer to mediate vector length conversion
            output_buffer_vector_name = f"{field_name}_output_buffer_vector"
            _, output_buffer_vector_desc = sdfg.add_array(output_buffer_vector_name, (1, ),
                                                          dace.vector(stream.dtype.base_type, vector_length),
                                                          storage=dace.StorageType.FPGA_Registers,
                                                          transient=True)
            output_buffer_vector_access = state.add_access(output_buffer_vector_name)
            ...
            write_tasklet = state.add_tasklet(f"write_{field_name}", {"buffer"}, {"pipe"}, write_condition)

            state.add_memlet_path(output_buffer_vector_access,
                                  write_tasklet,
                                  dst_conn="buffer",
                                  memlet=dace.Memlet(f"{output_buffer_vector_name}[0]"))
